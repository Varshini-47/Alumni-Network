...
const Chat = () => {
  ...
  useEffect(() => {
    if (!userId) return;
    axios
      .get(`http://localhost:8080/api/chat/recent/${userId}`, {
  ...
  const updateMessage = (id, newContent) => {
    axios
      .put(`http://localhost:8080/api/chat/edit/${id}`, newContent, {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8080/api/chat/history/${userId}/${receiverId}`)
          ...
          try {
            ...
            setRecentUsers((prevUsers) => {
              ...
              const exists = prevUsers.some(
                (user) => user.id == parsedMessage.senderId
              );
              console.log(exists);
              if (!exists) {
                ...
                axios
                  .get(
                    `http://localhost:8080/api/users/${parsedMessage.senderId}`
                  )
                  .then((response) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8080/api/searchchat?query=${searchQuery}`)
      .then((response) => {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => setRecentUsers(response.data))
      .catch((error) => console.error("Error fetching recent users"));
  }, [messages]);
    ...
    <div className="flex h-screen bg-chat-light dark:bg-chat-dark">
      ...
      <div className="w-64 bg-white shadow-soft h-[600px] overflow-hidden flex flex-col animate-fade-in">
        ...
        {isDropdownOpen && searchResults.length > 0 && (
          <div
            ref={dropdownRef}
            className="absolute top-[180px] left-4 bg-white border rounded-lg shadow-glass w-56 z-10 overflow-hidden animate-slide-up"
          >
          ...
          <div className="space-y-2 pr-2">
            {recentUsers.map((user, index) => (
              <div
                ...
                className={`p-3 rounded-lg cursor-pointer transition-all hover-scale ${
                  receiverId === user.id
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center ${
                      receiverId === user.id ? "bg-blue-600" : "bg-gray-200"
                    }`}
                  >
                    <User
                      className={`w-5 h-5 ${
                        receiverId === user.id ? "text-white" : "text-gray-500"
                      }`}
                    />
                  </div>
                  <div>
              ...
              <div className="space-y-3">
                {messages.map((msg, index) =>
                  ...
                  msg.receiverId === receiverId ? (
                    ...
                    >
                      <div
                        className={`relative chat-bubble flex items-center gap-2 px-4 py-2 rounded-lg text-black ${
                          msg.senderId === userId
                            ? "bg-blue-500"
