...
function Leaderboard() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setFilteredUsers(
          response.data.filter((user) => user.role === "ALUMNI")
        );
        setIsLoading(false);
      })
      .catch((error) => {
  ...
  const getMedal = (points) => {
    ...
    if (points >= 200)
      return {
        name: "Platinum",
        icon: <FaMedal className="text-gray-700" />,
    ...
    if (points >= 50)
      return {
        name: "Silver",
        icon: <FaMedal className="text-gray-500" />,
        color: "text-gray-500 font-semibold",
        bgColor: "bg-gray-100",
      };
    return {
      name: "Bronze",
      icon: <FaMedal className="text-orange-600" />,
      ...
      <div className="container mx-auto max-w-6xl">
        ...
        >
          ...
          <p className="text-center text-gray-600 max-w-2xl mx-auto mb-8">
            Recognizing our outstanding alumni who contribute to our community
                  ...
                  <table className="w-full border-collapse">
                    ...
                    <tbody>
                      {filteredUsers.map((user, index) => {
                        const { name, icon, color, bgColor } = getMedal(
                          user.points
                        );
                        return (
                          <motion.tr
                            ...
                            className={`cursor-pointer transition duration-300 hover:bg-indigo-50 ${
                          ...
                          >
                            ...
                            <td className="p-4">
                              <div
                                className={`flex items-center gap-2 ${color} ${bgColor} px-3 py-1.5 rounded-full w-fit shadow-sm`}
                              >
                                {icon}
                                <span>{name}</span>
                              </div>
